...
class OccupationStations {
  const OccupationStations({this.typeIds = const {}, this.workcenterIds = const {}});

  /// Workcenter type ids, as the run copied them in (§10.2). Empty is every
  /// type, including stations that have none.
  final Set<String> typeIds;

  /// Individual stations. Empty is all of them.
  final Set<String> workcenterIds;

  bool get isAll => typeIds.isEmpty && workcenterIds.isEmpty;

  bool includes({required String workcenterId, required String? typeId}) {
...
OccupationGraph? occupationGraph({
  ...
  OccupationStations stations = const OccupationStations(),
}) {
  ...
  final inView = {
    for (final id in capacityByStation.keys)
      if (stations.includes(workcenterId: id, typeId: typeOf[id])) id,
  };
